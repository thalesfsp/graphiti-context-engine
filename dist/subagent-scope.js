 ...
 */
function detectGroupFromAgentId(agentId) {
    const lower = agentId.toLowerCase();
    if (lower.includes('helix'))
        return 'helix';
    if (lower.includes('ringboost'))
        return 'ringboost';
    if (lower.includes('system') || lower.includes('validator'))
        return 'system';
    if (lower.includes('personal') || lower.includes('nova'))
        return 'personal';
 ...
 */
export function detectGroupsFromQuery(query) {
    ...
    if (detected.size === 0)
        return null; // No hints — use defaults
 ...
 */
export function detectGroupsFromChannel(meta) {
    if (!meta)
        return null;
 ...
 */
export function resolveGroupIds(sessionId, options) {
