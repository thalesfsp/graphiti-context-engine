...
//
// IMPORTANT: We call external LLM APIs directly (not via the OpenClaw gateway)
// because the gateway's /v1/chat/completions endpoint injects the agent's
// system prompt (Nova's persona), which overrides our extraction prompt and
// causes the model to respond in character instead of returning JSON.
//
// Provider priority: ANTHROPIC_OAT_KEY (Claude Haiku 4.5, subscription-backed, FREE)
//                  → XAI_API_KEY (Grok 4.1 Fast) → ANTHROPIC_API_KEY (Claude Haiku 4.5)
//                  → OPENAI_API_KEY (GPT-4o-mini)
// Override with GRAPHITI_EXTRACTION_URL + GRAPHITI_EXTRACTION_KEY + GRAPHITI_EXTRACTION_MODEL
 ...
 */
async function callProvider(provider: LLMProvider, text: string): Promise<PartialClaim[] | null> {
  ...
  return parsed
    .filter(c =>
      c.subject && typeof c.subject === 'string' &&
...
export async function extractClaims(
  ...
  if (text.length < 20) return { claims: [] };
