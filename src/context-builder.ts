...
export function buildContextAddition(claims: Claim[]): string {
  ...
  for (const claim of sorted) {
    ...
    const display = (claim as any).fact && claim.subject === ''
      ? (claim as any).fact
...
export const NOISE_INLINE_PATTERNS: RegExp[] = [
 ...
 */
export function stripNoisePatterns(text: string): string {
...
export function extractQueryFromMessages(messages: AgentMessage[]): string {
        ...
        text = m.content
          .map(block => {
            ...
            if (block && typeof block === 'object' && 'body' in block) {
              return (block as { body: string }).body;
            }
            return '';
          })
          .join(' ');
